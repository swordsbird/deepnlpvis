...
class DataLoader:
    ...
    def init_data(self):
        ...
        for i in tqdm(range(self.size)):
            ...
            self.contri.append(entropy.copy())
            ...
            if self.load_activation:
                ...
                self.word_activation.append(entropy)
    ...
    def get_polarity(self):
        self.polarity = []
        xi = self.threshold_xi
        for idx in range(self.size):
            old_s = self.all_old_s[idx]
            polarity_mat = []
            for layer in range(self.n_layer):
                s1 = old_s[self.main_index]
                s2 = old_s[self.second_index]
                ss = s1 + s2
                s1 = s1 / ss
                s2 = s2 / ss
                polarity_row = []
                for j, new_s in enumerate(self.all_new_s[idx][layer]):
                    ns1 = new_s[self.main_index]
                    ns2 = new_s[self.second_index]
                    nss = ns1 + ns2
                    ns1 = ns1 / nss
                    ns2 = ns2 / nss
                    d1 = ns1 - s1
                    d2 = ns2 - s2
                    if self.data_word[idx][j] == 'laughs':
                        print(layer, d1, s1)
                    ...
                    if d1 < -xi or d2 < -xi:
                        if d1 < -xi:
                            ...
                            polarity = -1
                    polarity_row.append(polarity)
                polarity_mat.append(polarity_row)
            self.polarity.append(np.array(polarity_mat))

    def get_all_layer_contri(self):
        ...
        for i in idxs:
            for t in self.data_token[i]:
                ...
                if w not in polarity:
                    polarity[w] = []
                if len(t[1]) == 1:
                    e = self.polarity[i][layer,t[1][0]]
                else:
                    sgn = self.polarity[i][layer,t[1]].sum()
                    e = sgn
                polarity[w].append(e)
        for t in polarity:
            tot = len(polarity[t])
            neg = len([x for x in polarity[t] if x > 0])
            pos = len([x for x in polarity[t] if x < 0])
            polarity[t] = {
