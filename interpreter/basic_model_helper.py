...
def load_basic_model(type='lstm', task='sst2', number = -1):
    ...
    if task == 'sst2':
        if type == 'lstm':
            ...
            model = basic_RNNClassifier(yaml.load(open(os.path.join(path_att_lstm, 'LSTM_basic_4_layer_768_hidden_pool_max.yaml'),'r')))
        ...
        if type == 'lstm':
            ...
            model = basic_RNNClassifier(yaml.load(open(os.path.join(path_lstm, 'LSTM_basic_4_layer_768_hidden_pool_max.yaml'),'r')))
...
def get_basic_embedding(model, types, Dataset: dataset, batchsize=-1, use_cuda = 'auto'):
    ...
    if use_cuda:
        all_embedding = all_embedding.cuda()
...
class lstm_basic_certain_hidden:
    ...
    def __init__(self, model, embedding, target_id, layer_num, device, isBatch=False):
        if not isinstance(model, basic_RNNClassifier):
